    ...
    def merge_invoices(self):
        ...
        for invoice in invoices:
            if invoice.state != 'draft':
                raise UserError(_(u'要合并的发票必须是草稿状态！'))
            if not partner_id:
                partner_id = invoice.partner_id
            elif partner_id != invoice.partner_id:
                raise UserError(_(u'要合并的发票必须是同一个供应商或客户！ '))
            if not account_id:
                account_id = invoice.account_id
            elif account_id != invoice.account_id:
                raise UserError(_(u'要合并的发票必须是同一个科目！ '))
            if not journal_id:
                journal_id = invoice.journal_id
            elif journal_id != invoice.journal_id:
                raise UserError(_(u'要合并的发票必须是同一个分录！ '))
            if not currency_id:
                currency_id = invoice.currency_id
            elif currency_id != invoice.currency_id:
                raise UserError(_(u'要合并的发票必须是同一种货币！ '))
            if not type:
                type = invoice.type
            elif type != invoice.type:
                raise UserError(_(u'要合并的发票必须是同一个类型！ '))
